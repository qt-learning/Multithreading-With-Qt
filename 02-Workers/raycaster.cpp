...
public:
  Ray(QVector3D start, QVector3D end)
      : m_start(std::move(start)), m_direction((end - m_start).normalized()) {}
...
QImage RayCaster::render(const QSize &viewport, qreal dpi, bool *cancelFlag) {
  const int minDimension = std::min(viewport.width(), viewport.height());
  QRectF screenRectangle(-0.5 * (viewport.width() / qreal(minDimension)),
                         -0.5 * ((viewport.height() / qreal(minDimension))),
                         viewport.width() / qreal(minDimension),
                         viewport.height() / qreal(minDimension));
  ...
  const QVector3D directionalLight =
      Ray(QVector3D(1, 1, 0), QVector3D(0, 0, 0)).direction();

  const QPointF screenDelta(
      screenRectangle.width() / (scaledViewport.width() - 1),
      screenRectangle.height() / (scaledViewport.height() - 1));
  ...
  for (int y = 0; y < scaledViewport.height(); ++y) {
    ...
    for (int x = 0; x < scaledViewport.width(); ++x) {
      Ray ray(QVector3D(screenRectangle.x() + x * screenDelta.x(),
                        screenRectangle.y() + y * screenDelta.y(), 0),
              QVector3D(screenRectangle.x() + x * screenDelta.x(),
                        screenRectangle.y() + y * screenDelta.y(), 1));

      const qreal a = ray.direction().x() * ray.direction().x() +
      ...
      const qreal c = sphere.center().x() * sphere.center().x() +
      ...
      if (delta >= 0) {
        ...
        const QVector3D pointOfCollision =
            ray.start() + tAtCollision * ray.direction();
        const QVector3D normalAtCollision =
            (pointOfCollision - sphere.center()).normalized();
        const qreal cosine =
            QVector3D::dotProduct(-directionalLight, normalAtCollision);
